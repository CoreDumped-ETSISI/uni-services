...
func generateImage(salas *salasResponse, disabled int, stream io.Writer) {
	w := 700
	h := 500
	salah := float64(h / 5.0)
	totalHours := 22
...
func RenderSalas(body io.Reader, response io.Writer) error {
	var salas salasResponse
	err := json.NewDecoder(body).Decode(&salas)
	...
	generateImage(&salas, timeToIndex(now.Hour(), now.Minute())+1, response)
	return nil
}
