...
func generateImage(horario [][]class, stream io.Writer) {
	margin := 50.0
...
func RenderImage(class string, body io.Reader, response io.Writer) error {
	var hor horarios
	err := json.NewDecoder(body).Decode(&hor)

	if err != nil {
		return err
	}

	generateImage(hor, response)
	return nil
}
